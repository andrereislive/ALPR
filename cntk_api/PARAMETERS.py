...
rootDir = os.path.dirname(os.path.realpath(sys.argv[0]))+ "/cntk_api"


if not (os.path.exists(rootDir)): # para evitar problema entre rodar no Rest e rodar diretamente o 
...
ss_max_merging_iterations = ctm_ss_max_merging_iterations#20  #
ss_nmsThreshold = ctm_ss_nmsThreshold #0.85          # non-maxima surpression threshold run after selective search
grid_nrScales = ctm_grid_nrScales #7                     # uniform grid ROIs: number of iterations from largest possible ROI to smaller ROIs
grid_aspectRatios = ctm_grid_aspectRatios #[1.0, 2.0, 0.5]    # uniform grid ROIs: allowed aspect ratio of ROIs
grid_downscaleRatioPerIteration = ctm_grid_downscaleRatioPerIteration# 1.5  # uniform grid ROIs: relative ROI width/height reduction per iteration, starting from largest possible ROI
...
cntk_lr_per_image  = [0.01] * 10 + [0.001] * 5 + [0.0001]  #lr = learning rate per image
...
vis_decisionThresholds = {'svm' : 3.0,  # Reject detections with low confidence, used only in 5_visualizeResults
                          'nn' : None}
...
if datasetName.startswith("grocery"):
    ...
    print("erro de classificador! Favor me implementar")
    ERROR
